...
class TestcaseAgentState(TypedDict, total=False):
    ...
    test_strategy: dict         # {strategies: [{module_name, methods, case_count_range, coverage_targets}]}
...
| plan_test_strategy | 测试架构师 | 分析结果 | `{strategies: [{module_name, methods, case_count_range, coverage_targets}]}` | kimi-k2.5 |
...
参数：
  ...
  mode: string         — 生成模式（comprehensive/balanced/quality，默认 comprehensive）
