...
# 从 redis-py 4.1.0+ 开始，集群支持已集成到官方库中
...
class RedisManager:
    ...
    def get_cluster(address: str):
        """
        获取集群连接池
        使用 redis-py 官方集群支持（4.1.0+）
        :param address: 集群节点地址，格式：host1:port1,host2:port2,...
        :return: RedisCluster 客户端实例
        """
        try:
            nodes = address.split(",")
            cluster_nodes = [
            ...
            logger.info(f"使用 redis-py 官方集群支持连接 Redis 集群: {address}")
            return client
