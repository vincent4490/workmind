...
1. 应该使用哪些测试设计方法
2. 用例建议预算与覆盖优先级（预算用于控冗余，不是硬性上下限）
3. 重点覆盖维度
4. 优先级分布建议
...
{{
    "strategies": [
        {{
            ...
            "case_budget": {{
                "target": 12,
                "suggested_range": [8, 15],
                "flex_policy": "建议预算，不是硬性上下限；覆盖关键规则优先，简单模块可少于预算，高风险模块可超过预算"
            }},
...
要求：
1. case_budget 是建议预算：用于防止冗余，不用于机械凑数或截断关键风险
2. simple 模块不需要所有测试方法
3. critical 模块必须包含安全/权限相关覆盖目标
4. focused 模式预算整体偏紧，comprehensive 可放宽；但最终以关键规则/风险覆盖是否完整为准
5. 只返回 JSON，不要有 ```json 标记
"""
...
3. 该模块的测试策略（测试方法、用例建议预算、覆盖目标、dedupe_policy）
4. 全局业务规则和隐含规则
...
2. 严格遵循策略中指定的测试方法；case_budget 是建议预算，不是硬性上下限
3. steps 字段中多个步骤用 \\n 分隔
4. 每条用例必须包含至少一个 coverage_points，且 rule_id 必须来自分析/全局/隐含规则
5. 不得输出重复验证同一 dedupe_key 的多条用例（除非 scenario_type 不同）
6. 覆盖优先：关键规则/高风险场景必须覆盖；低风险或重复场景可以少于预算，不得为了凑数量生成低价值重复用例
7. 只返回 JSON，不要有 ```json 标记
