...
```json
{
  ...
  "markdown_full": "# 竞品分析报告\\n## 对比矩阵概述\\n（≥200字正文，包含：对比矩阵概述 → SWOT → 核心差异化机会 → 产品建议 → 总结结论）\\n## SWOT\\n\\n## 核心差异化机会\\n\\n## 产品建议\\n\\n## 总结结论\\n",
  "recommendations": ["建议1：..."],
...
7. confidence_score 标准：信息完整度 60%，分析深度 40%
  8. markdown_full 必须≥200字，内容结构至少包含：对比矩阵概述 → SWOT → 核心差异化机会 → 产品建议与结论
  9. markdown_full 不得使用 "..." 作为占位符，必须输出真实正文
  10. markdown_full 作为 JSON 字符串必须是“单行可解析”的：不要在 markdown_full 字符串里直接输出真实换行，换行请使用 `\\n`
"""
